module.exports.run = async (client, message, args, username, channel) => {
  ...
  const suggestTotal = await total.rows.pop().suggestid;
  
  if (!args[0]) return client.say(channel, `${username}, insira uma sugestão :/`)
  
  await db.query(`INSERT INTO luulbot_suggests(userchannel, usersuggest, suggestid) VALUES('${username}','${args.join(" ")}', '${suggestTotal + 1 || 1}')`)
